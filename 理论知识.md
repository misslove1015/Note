...
* **事件的响应**  
  ...
  * UIApplication
  ...
  在事件的响应中，如果某个控件实现了touche方法，则这个事件将由该控件来接受，如果调用了[super touche]，就会将事件顺着响应者链条往上传递，传递给上一个响应者；接着就会调用上一个响应者的touche方法。我们可以在touch方法里调用[super touches….]来实现多个视图响应一个事件。
